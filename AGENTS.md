...
- **SPA only after `npm run build`**: `spa_index` reads `STATIC_ROOT/web/index.html` and 404s "Web pública no construida" otherwise (`apps/core/views.py:94-100`). The SPA build artifacts (`backend/static/web/`) **are committed** (forced `git add -f`, git-ignored) so the public catalog serves at `/` on Render — after ANY frontend change, run `cd frontend && npm run build` and commit `backend/static/web/` again. Deep links (`/catalogo`, `/vehiculo/<id>`, `/tasacion`, `/contacto`) hit the `spa_index` catch-all and work via vue-router history mode.
...
| Build frontend | `cd frontend && npm run build` → `backend/static/web/` (**commit the artifacts** with `git add -f` — the public web serves from them) |
...
- **Render URLs + shared DB**: real service URLs are `eurocar-staging-jdx2.onrender.com` (staging; login `tester` / `TestMadrid2024!`) and `eurocar-jdx2.onrender.com` (production). **`eurocar.onrender.com` AND `eurocar-staging.onrender.com` (no `-jdx2`) are ghost/leftover services with stale `DATABASE_URL`s — login POST and any DB endpoint 500 there; never use them for work or diagnostics.** Both web services share the SAME PostgreSQL (`eurocar-db-jdx2`, Basic-256mb): any `migrate` deployed on either branch affects both apps — consider splitting when staging data matters. The root path `/` returns 404 on Render (SPA never built there) — the ERP lives under `/erp/`; a root 404 is expected, not a bug. If any DB-touching endpoint 500s while `/api/ping/` is 200, suspect a stale `DATABASE_URL` env (free-tier DBs expire ~30 days and get deleted) or a deploy that didn't actually apply — Render can show "Live" while still serving the old instance; fix with *Manual Deploy → Clear build cache & deploy* after updating the env var (staging auto-deploys from `staging`; production deploys manually from `master`).
...
- **Staging media stays local/ephemeral intentionally** (cost saving) — `staging.py` has no S3 block; don't "fix" it. The SPA artifacts (`backend/static/web/`) are committed, so the public catalog serves at `/` on staging/prod; keep `collectstatic` in the Render build (it copies `static/web/` into `staticfiles/web/` for `spa_index`).
- **WhiteNoise without manifest**: `STATICFILES_STORAGE = CompressedStaticFilesStorage` (staging + production) — intentional. The Vite `index.html` references assets by name (`/static/web/assets/...`); the manifest storage renames files and would break the SPA. Django's own statics lose hash cache-busting (cosmetic).
